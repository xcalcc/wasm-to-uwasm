...
public:
  ...
  OPTYPE_SZ    OpSz1(void)     const      { return _size1;   }
  OPTYPE_SZ    OpSz2(void)     const      { return _size2;   }
  ...
  WASM_INST(const char *name, UINT8 encode, OPTYPE_SZ sz1, OPTYPE_SZ sz2,
            INST_CLASS ins, UINT8 numopr) :
            _name(name), _encode(encode), _size1(sz1), _size2(sz2),
            _class(ins), _numopr(numopr), _pc(0) {}
  UINT8 Print_opr (FILE *file, OPTYPE_SZ opr_size, IO_CBUFFER pc);
  INT32 Print(FILE *file = stdout);
  INT32 Print(IO_CBUFFER pc, FILE *file = stdout);
  INT32 Print(IO_CBUFFER pc, UINT32 ins_cnt, FILE *file = stdout);
  INT32 Print(UINT32 ins_cnt, FILE *file);
  UINT8 Emit_opr (FILE *file, UINT8 opr_size, IO_CBUFFER pc);
  INT32 Emit_binary(IO_CBUFFER pc, FILE *file = stderr);
  UINT16 Read_inst_size(IO_CBUFFER pc);
  UINT64 Read_opr(UINT8 opr_size, IO_CBUFFER pc);
...
public:
    WASM_SECTION(SECTION_ID id, UINT32 size, UINT32 num) : _id(id), _size(size), _num(num) {};

    SECTION_ID          Id(void)      const   { return _id;   }
...
private:
    UINT32 _num_param;
    std::vector<OPTYPE_SZ> _type_param;
    UINT32 _num_res;
    std::vector<OPTYPE_SZ> _type_res;
public:
    FUNC_TYPE(UINT32 numParam,      std::vector<OPTYPE_SZ> typeParam,
              UINT32 numRes,        std::vector<OPTYPE_SZ> typeRes) :
    ...
    const   std::vector<OPTYPE_SZ> &TypeParam() const { return _type_param; }
    UINT32  NumRes()                            const { return _num_res;    }
    const   std::vector<OPTYPE_SZ> &TypeRes()   const { return _type_res;   }
};
