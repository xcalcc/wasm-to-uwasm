...
Result BinaryReader::ReadFunctionBody(Offset end_offset) {
  // Reroute the work to internal routine in test.cpp
  if (Get_conv_opt().Is_conv_in_read()) {
    FILE_CONTEXT ctx;
    ctx.Set_kind(FILE_WASM);
    Read_insts((UINT8 *) state_.data + state_.offset, end_offset - state_.offset, ctx);
...
Result BinaryReader::ReadCodeSection(Offset section_size) {
  ...
  for (Index i = 0; i < num_function_bodies_; ++i) {
    ...
    Is_Trace(Tracing(TP_BIN_READ),
             (TFile, "Reading code section, end = (hex = %0#lx, dec = %lu) \n",
              end_offset, end_offset));
    CALLBACK(OnFunctionBinaryRead, func_index, state_.data, state_.offset, end_offset);
